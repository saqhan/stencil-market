...
export class SCntMarketLeftMenuCatalog implements ComponentInterface {
    ...
    return (
      <div>
        <div
          ...
          class={this.openedLeftMenu ? 'drawer-backdrop  opened ' : 'drawer-backdrop '}
          ref={(el) => (this.leftMenuTag = el)}
          onClick={(event) => this.clickOnLeftMenuOverlayHandler(event)}
        ></div>
        <div
          class={this.openedLeftMenu ? 'drawer-left drawer-transition opened ' : 'drawer-left drawer-transition '}
   ...
   **/
  // public checkSales(){
  //   this.leftMenuCatalogArr.map(item => {
  //     if (item.sales) {
  //       this.isShowSalesBlock = true;
  //       console.log('sasasas');
  //     }
  //   })
  // }
